...
export function Panel({
  ...
  return (
    ...
    >
      ...
      {/*
       * O corpo carrega o proprio respiro: o cabecalho tem borda propria, entao o
       * padding nao pode viver na moldura. Quando rola, o topo fica sem padding de
       * proposito — com ele, o thead sticky gruda abaixo da faixa e o conteudo passa
       * por tras, aparecendo no vao.
       */}
      <div
        className={`min-h-0 min-w-0 flex-1 ${scroll ? 'scroll-area px-4 pb-4' : 'p-4'} ${bodyClassName}`}
      >
        {children}
      </div>
    </section>
...
export function Alert({
  ...
  const tones = {
    ...
    danger: 'border-[color-mix(in_oklab,var(--neg)_40%,transparent)] bg-[color-mix(in_oklab,var(--neg)_7%,transparent)] text-neg',
    success: 'border-[color-mix(in_oklab,var(--pos)_40%,transparent)] bg-[color-mix(in_oklab,var(--pos)_8%,transparent)] text-pos',
  } as const
...
export function StatusChip({
  ...
  const tones = {
    ...
    won: 'border-[color-mix(in_oklab,var(--pos)_40%,transparent)] bg-[color-mix(in_oklab,var(--pos)_14%,transparent)] text-pos',
